...
{    
    ...
    /*确保所接收的是我所发的的ICMP的回应*/     
	printf("type %d pid %d %d byte from %s: icmp_seq=%u ttl=%d rtt=%.3f ms\n",icmp->icmp_type,icmp->icmp_id,len,inet_ntoa(from.sin_addr),icmp->icmp_seq,ip->ip_ttl,rtt); 
...
{      
	...
	{      
        ...
        //}
                 //printf("recform ipaddr:%s",from.sin_addr.s_addr);
...
{     
	...
	printf("detect_ipc_online pid:%d \n",pid);
